...
final class AppUpdaterController {
    ...
    private let userDriver: DettoUserDriver

    init() {
        let hostBundle = Bundle.main
        userDriver = DettoUserDriver(hostBundle: hostBundle, delegate: nil)
        updater = SPUUpdater(
    ...
    private func presentStartupError() {
        ...
        alert.informativeText = "The updater failed to start. Please verify you have the latest version of Detto and contact the developer if the issue persists."
        alert.runModal()
    }
}

@MainActor
final class DettoUserDriver: SPUStandardUserDriver {
